...
/**
 * Tests that InitiatedStatusProcessor maps the DTO to an entity, calls
 * {@link TransactionDao#updateTransaction}, and returns the original DTO.
 */
@ExtendWith(MockitoExtension.class)
class InitiatedStatusProcessorTest {

    @Mock private TransactionDao transactionDao;
    @Mock private ModelMapper modelMapper;

    private InitiatedStatusProcessor processor;

    @BeforeEach
    void setUp() {
        processor = new InitiatedStatusProcessor(transactionDao, modelMapper);
    }

    @Test
    void processStatus_updatesEntityAndReturnsDto() {
        TransactionDto dto = new TransactionDto();
        dto.setTxnReference("init-123");
