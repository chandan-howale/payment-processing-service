...
/**
 * Tests that ApprovedStatusProcessor maps the DTO to an entity,
 * calls {@link TransactionDao#updateTransaction}, and returns the DTO.
 */
@ExtendWith(MockitoExtension.class)
class ApprovedStatusProcessorTest {
    ...
    private ApprovedStatusProcessor processor;

    @BeforeEach
    void setUp() {
        processor = new ApprovedStatusProcessor(transactionDao, modelMapper);
    }

    @Test
    void processStatus_updatesEntityAndReturnsDto() {
        TransactionDto dto = new TransactionDto();
        dto.setTxnReference("approved-123");
